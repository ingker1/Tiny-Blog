                    ...
                    <tr v-if="editingArticleIndex !== index"
                        ...
                        <td>
                            <div style="display: flex;">
                                <button @click="recoverArticle(article)">还原</button><br>
                                <button @click="deleteArticle(article.articleId)" class="delete">删除</button>
                            </div>
                        </td>
                    </tr>
                </template>
...
<script setup>
    ...
    const recoverArticle = async (article) => {
        try {
            let articleContent = '';
            ...
            await axios.put(`http://localhost:8080/admin/comments/recycle/${article.articleId}`)
            .catch(error => {
                console.error(error);
            });
    ...
    const deleteArticle = (articleId) => {
        ...
        if (confirmed) {
            axios.delete(`http://localhost:8080/admin/articles/${articleId}`)
            .then(() => {
                alert('删除成功');
                loadArticles(); // 刷新文章列表
            })
    ...
    const changePage = (page) => {
        if (page > 0 && page <= totalPages.value) {
            ...
            router.push({ path: '/admin/trash', query: queryParams.value });
            loadArticles(); 
        } else {
            alert('页码无效');
        }
